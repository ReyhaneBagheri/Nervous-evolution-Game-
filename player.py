...
class Player(pygame.sprite.Sprite):
    def __init__(self, game_mode):
        ...
        if self.game_mode == "Neuroevolution":
            ...
            layer_sizes = [8, 19, 2]  # TODO (Design your architecture here by changing the values)
    ...
    def think(self, screen_width, screen_height, obstacles, player_x, player_y):
        ...
        if (len(obstacles) > 2):
            vectors0 = obstacles[0]['x']
            vectors1 = obstacles[0]['y']
            vectors2 = obstacles[1]['x']
            vectors3 = obstacles[1]['y']
            vectors4 = obstacles[2]['x']
            vectors5 = obstacles[2]['y']
            vectors6 = player_x
            vectors7 = player_y
        ...
        nn_input_vector = np.array((vectors0, vectors1, vectors2, vectors3, vectors4, vectors5,
                                 vectors6, vectors7))


        # reshaped form. Has two elements. path[0][0] is for the left side and path[1][0] is for the right side
        norm = np.linalg.norm(nn_input_vector)
        path = self.nn.forward(nn_input_vector / norm)
